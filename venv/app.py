...
def generate():
    ...
    # Handle text-to-speech if requested
    if 'play_voice' in request.form:
        text_to_speech(translated_text, language)

    # Instead of downloading directly, save the text for later download
    lyrics_file_path = None
    if 'download' in request.form:
        filename = "generated_lyrics.txt"
        with open(filename, "w", encoding="utf-8") as file:
            file.write(translated_text)
        lyrics_file_path = filename  # Pass the file path for downloading later
